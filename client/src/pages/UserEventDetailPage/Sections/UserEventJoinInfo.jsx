...
const waitingMent = (eventId, answerId) => {
  ...
  useEffect(() => {
    dispatch(answerRowNum(body)).then((response) => {
  ...
  return (
    "대기 번호 " + myNum + "번 / 현재 내 앞 대기팀 " + frontWaitingNum + "팀"
  );
};
...
const UserEventJoinInfo = (props) => {
  ...
  return (
    <>
      ...
      <Box sx={{ mb: 1 }}>
        {moment(eventDetail.participantDate).format("YYYY-MM-DD HH:mm:ss.SSS")}
      </Box>
      <AnswerList
        hideRequired
